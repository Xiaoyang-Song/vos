...
class DenseNet3GP(nn.Module):
    def __init__(self, depth, num_classes, growth_rate=12,
        ...
        #     self.fc = nn.Linear(in_planes, num_classes)
        # else:
        #     self.fc = RouteFcUCPruned(
        #         in_planes, num_classes, topk=k, info=info)
    ...
    def forward_virtual(self, x):
        ...
        out = out.view(-1, self.in_planes)
        # breakpoint()
        penultimate = self.relu(self.fc0(out))
        return self.fc(penultimate), out

    def features(self, x):
        if self.normalizer is not None:
            x = x.clone()
            x[:, 0, :, :] = (
                x[:, 0, :, :] - self.normalizer.mean[0]) / self.normalizer.std[0]
            x[:, 1, :, :] = (
                x[:, 1, :, :] - self.normalizer.mean[1]) / self.normalizer.std[1]
            x[:, 2, :, :] = (
                x[:, 2, :, :] - self.normalizer.mean[2]) / self.normalizer.std[2]
